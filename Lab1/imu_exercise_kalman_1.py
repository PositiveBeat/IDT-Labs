...
for line in f:
	...
	if new_gyro_data available:
		predAngle_x = estAngle_x + gyro_x * T
		predAngle_y = estAngle_y + gyro_y * T
		predAngle_z = estAngle_z + gyro_z * T
		gyroVarAcc = gyroVarAcc + gyroVar
		predVar = estVar + gyroVarAcc * T
		estAngle_x = predAngle_x
		estAngle_y = predAngle_y
		estAngle_z = predAngle_z
		estVar = predVar
		#end if something?


	# Kalman correction step (we have new data in each iteration)
	if new_lidar_data available:
		K = predVar / (predVar + lidarVar)
		corrAngle_x = predAngle_x + K * (lidarAngle - predAngle_x)
		corrAngle_y = predAngle_y + K * (lidarAngle - predAngle_y)
		corrAngle_z = predAngle_z + K * (lidarAngle - predAngle_z)
		corrVar = predVar * (1 - K)
